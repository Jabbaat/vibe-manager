...
app = FastAPI(title="De Eddiecool Vibe_Agent 7.3")
...
# --- HET BREIN ---
SYSTEM_INSTRUCTION = """
JIJ BENT: De Eddiecool Vibe_Agent (Versie 7.3 - Clean Reader).
JE KENMERKEN:
1. Je spreekt Nederlands.
2. Je bent enthousiast, direct, creatief en 'Neobrutalist' in stijl.
3. Je bent een ONDERZOEKER. Als ik een URL stuur, lees je de inhoud (via de context) en gebruik je die FEITELIJK.
4. Je kunt bestanden maken (Creator Mode).
5. Je kunt zien (Vision).

BELANGRIJK:
- Gebruik [[FILE: naam]] ... [[ENDFILE]] voor bestanden.
- Als je informatie uit een link haalt, wees dan specifiek en gedetailleerd.
- Houd antwoorden beknopt zodat de voorlees-stem niet te lang aan het woord is.
"""

# --- FUNCTIE: SMART URL LEZER (VIA JINA) ---
def fetch_web_content(url):
    """Gebruikt r.jina.ai om ook Javascript/Dynamische websites te kunnen lezen."""
    try:
        # 1. URL SCHOONMAKEN (Cruciaal voor brackets!)
        # We halen haakjes [], (), komma's en punten weg aan het einde van de URL
        clean_url = url.strip("[](),.!?;:'\"")
        
        print(f"🌍 Bezoeken van: {clean_url}")
        ...
        # Timeout iets verhoogd naar 30 seconden voor trage sites
        response = requests.get(jina_url, headers=headers, timeout=30)
        
        clean_text = response.text
        
        if "<title>404" in clean_text or "Not Found" in clean_text:
             return f"FOUT: De pagina {clean_url} bestaat niet (404)."
        ...
        return f"Kon de website niet lezen: {str(e)}"
...
async def chat_with_file(text: str = Form(...), file: Optional[UploadFile] = File(None)):
    try:
        ...
        # --- 1. SLIMME URL DETECTIE (AUTO CLEAN) ---
        # Zoek alles wat op een url lijkt
        urls = re.findall(r'https?://[^\s]+', text)
        web_content = ""
        
        if urls:
            web_content = "\n\n--- INHOUD VAN GEVONDEN URLS ---\n"
            for url in urls:
                # Hier gebeurt het: fetch_web_content maakt hem nu zelf schoon!
                content = fetch_web_content(url)
                web_content += f"URL: {url}\nINHOUD: {content}\n\n"
            web_content += "--- EINDE URL INHOUD ---\n"

        prompt_text = text + web_content
        if prompt_text: current_parts.append(types.Part(text=prompt_text))
...
<html>
<head>
    <title>De Eddiecool Vibe_Agent 7.3</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .layout {
            ...
            background: var(--paper-white); border: 6px solid var(--deep-black);
            box-shadow: 12px 12px 0px var(--deep-black);
            display: flex; flex-direction: column; overflow: hidden; position: relative;
        }

        .header {
            background: var(--acid-green); border-bottom: 6px solid var(--deep-black);
            padding: 20px; display: flex; justify-content: space-between; align-items: center; flex-shrink: 0;
        }

        h1 { margin: 0; font-size: 1.5rem; font-weight: 900; text-transform: uppercase; letter-spacing: -1px; color: var(--deep-black); }

        .voice-toggle { 
            font-weight: 900; background: var(--paper-white); padding: 8px 12px; 
            border: 4px solid var(--deep-black); cursor: pointer; box-shadow: 4px 4px 0px var(--deep-black);
            font-size: 0.9rem;
        }
        .voice-toggle.active { background: var(--hot-pink); }

        .chat { flex: 1; overflow-y: auto; padding: 20px; background: var(--paper-white); display: flex; flex-direction: column; gap: 20px; min-height: 0; }
        
        .msg { padding: 15px 20px; max-width: 85%; font-size: 1.1rem; line-height: 1.5; border: 4px solid var(--deep-black); font-weight: 700; }
        ...
        .controls { padding: 20px; background: var(--hot-pink); border-top: 6px solid var(--deep-black); display: flex; gap: 10px; align-items: stretch; flex-shrink: 0; }
        input[type="text"] { flex: 1; padding: 15px; font-family: 'Courier New', monospace; font-weight: 900; font-size: 1.1rem; border: 4px solid var(--deep-black); outline: none; box-shadow: 4px 4px 0px rgba(0,0,0,0.2); min-width: 0; }

        button, .file-label, .mic-btn { background: var(--deep-black); color: var(--paper-white); border: 4px solid var(--deep-black); font-weight: 900; cursor: pointer; box-shadow: 6px 6px 0px var(--paper-white); display: flex; align-items: center; justify-content: center; }
        ...
        button:active, .file-label:active, .mic-btn:active { transform: translate(4px, 4px); box-shadow: 2px 2px 0px var(--paper-white); }
        .download-box { background: var(--paper-white); border: 4px solid var(--deep-black); padding: 15px; margin-top: 10px; display: flex; align-items: center; gap: 15px; }
        .download-btn { background: var(--deep-black); color: white; text-decoration: none; padding: 10px; font-weight: 900; }

        @media (max-width: 600px) {
            body { padding: 0; background: var(--hot-pink); }
            .layout { width: 100%; height: 100dvh; border: none; box-shadow: none; max-width: none; }
            .header { padding: 10px; } h1 { font-size: 1.1rem; } .voice-toggle { padding: 5px; font-size: 0.8rem; }
            .controls { padding: 10px; flex-wrap: nowrap; gap: 5px; background: var(--hot-pink); }
            .file-label { width: 44px; padding: 0; justify-content: center; font-size: 1.2rem; flex-shrink: 0; }
            .mic-btn { width: 44px; font-size: 1.2rem; flex-shrink: 0; }
            .send-btn { width: auto; padding: 0 15px; font-size: 1rem; flex-shrink: 0; }
            input[type="text"] { flex: 1; min-width: 0; margin: 0; font-size: 16px; padding: 10px; }
        }
...
<body>
    <div class="layout">
        <div class="header">
            <h1>De Eddiecool Vibe_Agent 7.3</h1>
        ...
        <div class="chat" id="chat">
            <div class="msg model">
                <strong>EDDIECOOL VIBE AGENT 7.3 READY. 💎</strong><br>
                Clean Reader Active.<br>
                Ik maak je linkjes nu automatisch schoon! 🧼<br>
                🚀
            </div>
        </div>
        ...
        <div class="controls">
            ...
            <input type="text" id="in" placeholder="Link of vraag..." onkeypress="if(event.key==='Enter') send()">
    ...
    <script>
        ...
        function toggleVoice() {
            ...
            if(voiceEnabled) { el.innerText = "🔊 AAN"; el.classList.add("active"); } 
            else { el.innerText = "🔇 UIT"; el.classList.remove("active"); synthesis.cancel(); }
        }
        ...
        function speak(text) {
            ...
            let cleanText = text.replace(/```[\s\S]*?```/g, " [Code blok] ").replace(/\[\[FILE:.*?\]\][\s\S]*?\[\[ENDFILE\]\]/g, " [Bestand klaar] ").replace(/\*/g, "");
            let utterance = new SpeechSynthesisUtterance(cleanText);
            utterance.lang = 'nl-NL'; utterance.rate = 1.1;
            synthesis.speak(utterance);
        }
        async function send() {
            let i = document.getElementById("in"); let f = document.getElementById("fileInput"); let c = document.getElementById("chat");
            let t = i.value.trim(); let file = f.files[0];
            if(!t && !file) return;
            synthesis.cancel();
            ...
            c.innerHTML += `<div class="msg model" id="${loadId}">... LEZEN & DENKEN ...</div>`;
            c.scrollTop = c.scrollHeight;
            const formData = new FormData(); formData.append("text", t); if (file) formData.append("file", file);
            try {
                ...
                f.value = ""; if(d.raw_text) speak(d.raw_text);
